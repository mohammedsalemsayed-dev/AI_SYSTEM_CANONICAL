...
Every existing document is kept. Each Markdown document now carries a **Cross-reference
header** block directly under its title (Role / Authority / Upstream / Downstream / Wiring),
so the design is navigable as one connected whole.

| Document | Role | Cross-ref header |
|---|---|---|
| `00_START_HERE/README_FOR_CLAUDE_CODE.md` | read order + authority rules | applied |
| `02_.../CANONICAL_CONTEXT.md` | principles in brief | applied |
| `02_.../IMPLEMENTATION_STATUS.md` | built vs. active boundary | applied |
| `02_.../REQUIREMENT_TRACEABILITY.md` | **the connective index** (see below) | applied |
| `03_.../prior_foundation/README.md` | package overview + invariants | applied |
| `03_.../docs/00_MASTER_SPEC.md` | frozen architecture + principles | applied |
| `03_.../docs/architecture/END_TO_END_ARCHITECTURE.md` | request lifecycle + state machine | applied |
| `03_.../docs/contracts/CORE_CONTRACTS.md` | canonical record schemas | applied |
| `03_.../docs/decision_history/ACTIVE_DECISIONS.md` | D1–D18, append-only | applied |
| `03_.../docs/implementation/IMPLEMENTATION_PLAN.md` | milestones A–I | applied |
| `03_.../docs/testing/ACCEPTANCE.md` | release gates | applied |
| `03_.../docs/operations/CLAUDE_CODE_INSTRUCTIONS.md` | coding-agent working rules | applied |
| `03_.../docs/source_basis/README.md` | provenance note | applied |
| `03_.../docs/FINAL_AUDIT.md` | preserved-requirements checklist | applied |
| `04_.../CLAUDE_CODE_OPERATING_PROTOCOL.md` | per-change protocol + definition of done | applied |
| `04_.../FINAL_RECONCILIATION.md` | packaging-correction rationale | applied |
| `DESIGN_TIGHTENING.md` (this) | wiring + concrete detail | linked from each gap it fills |
| `Complete Claude-Code Spec` / `Master Blueprint` (`.docx`) | authoritative source / rationale | binary — referenced from the header blocks and the index, not edited in place |

**The connective index** is [`02_CONTEXT_AND_TRACEABILITY/REQUIREMENT_TRACEABILITY.md`](02_CONTEXT_AND_TRACEABILITY/REQUIREMENT_TRACEABILITY.md):
one row per requirement with columns
`requirement | authoritative source | architecture target | contracts touched | milestone
(§10) | acceptance category | status`. That single table is where "everything connects" is
