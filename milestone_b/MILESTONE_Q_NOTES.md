...
| Matrix runner | `tests/fault/run_fault_suite.py` | Runs every `(kind × injection point)` over a canonical calc-bug task, checks all three invariants, writes `milestone_b/FAULT_FINDINGS.md`. Offline (scripted providers). **14/14 pairs PASS.** |
