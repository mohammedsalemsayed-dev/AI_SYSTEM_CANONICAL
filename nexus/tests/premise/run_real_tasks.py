...
Auth (default: no API key, no per-token spend):
  ...
  - SLICE_LLM=anthropic — raw Messages API; put `ANTHROPIC_API_KEY=...` (and
    optionally `SLICE_LLM_MODEL=...`) in `nexus/.env.local` (git-ignored,
...
def _load_env_local() -> None:
    ...
    for candidate in (Path("nexus/.env.local"), Path(".env.local")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if llm_kind == "anthropic" and not (
        ...
        print(
            ...
            "ANTHROPIC_API_KEY=... in nexus/.env.local, or use the default "
...
def _write_findings(rows: list[dict]) -> None:
    ...
    lines += [
        ...
        "  start pulling from design-notes section 14 (security seam 14.3, then Tier-A sandbox 14.6).",
