...
| Futuristic desktop UI | D10 / UI | Tauri / React app | event-log derived views (§11.2) | H (parallel from B) | Integration | FOUNDATION (slice: 6 §11.2 read-model folds + loopback HTTP/JSON API + SSE event stream + a wired no-build frontend replacing the static mock; opt-in gated task submit; Tauri packaging is the remaining step) |
