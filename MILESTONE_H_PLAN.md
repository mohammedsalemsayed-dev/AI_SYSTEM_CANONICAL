...
> **Cross-reference**
> - Role: Build plan for the event-stream API over the append-only event log, the §11.2-derived read models (task timeline, agents panel, system-health strip, metrics, routing), and a working wired frontend.
> - Authority: Implementation plan; subordinate to the Complete Claude-Code Spec and [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md).
> - Upstream (consumes): [IMPLEMENTATION_PLAN.md](03_IMPLEMENTATION_FOUNDATION/prior_foundation/docs/implementation/IMPLEMENTATION_PLAN.md) milestone H; [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §11.2 (observability — every view derived and rebuildable from the event log), §15.2 (the app owns the desktop shell + conversation + progressive-disclosure panels), §1 (the object flow the timeline renders).
> - Downstream (depended on by): nothing in the build order — H is a consumer of the event log. It makes B–I observable.
> - Predecessors: A (the event log). Buildable in parallel from B; scheduled here after the control plane (B–I) is complete so every panel has real data to show. Continues the `milestone_b/` tree.
...
| 14 | Regression; `milestone_b/MILESTONE_H_NOTES.md`; update [IMPLEMENTATION_STATUS.md](02_CONTEXT_AND_TRACEABILITY/IMPLEMENTATION_STATUS.md) + the [connective index](02_CONTEXT_AND_TRACEABILITY/REQUIREMENT_TRACEABILITY.md); rehash `MANIFEST.json`. |
...
- `milestone_b/MILESTONE_H_NOTES.md`.
