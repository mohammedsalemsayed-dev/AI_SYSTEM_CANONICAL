...
> **Cross-reference**
> - Role: Close a real §14.1 gap — a code-edit step is policy-checked once, with the *workspace root* as its only path, so the risk-class approval gate (`*auth*`, `*/migrations/*`, `*secret*`, `*/payments/*`, …) **never fires for the files a build actually touches**. Milestone V re-runs the existing Policy Engine once per changed file.
> - Authority: Implementation plan; subordinate to the Complete Claude-Code Spec and [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md).
> - Upstream (consumes): [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §1 (`ActionProposal` → `PolicyDecision` per action), §5-C (deterministic enforcement), §14.1 (permanent human-approval gate for risk-class paths); [MILESTONE_C_PLAN.md](MILESTONE_C_PLAN.md) (`PolicyEngine`, `rule_risk_class_needs_approval`, `rule_path_out_of_scope`, `rule_operation_not_granted`); the S/T/U notes' recurring "route the Builder through the dispatcher" deferral.
> - Downstream: the audit log gains one `POLICY_DECISION` per file a build changes; a build that edits an auth file or a migration now pauses for approval as the design requires.
> - Predecessors: C (policy engine + capability grant), B (`_run_step`). Continues the `milestone_b/` tree.
...
| 7 | Regression; `milestone_b/MILESTONE_V_NOTES.md`; update [IMPLEMENTATION_STATUS.md](02_CONTEXT_AND_TRACEABILITY/IMPLEMENTATION_STATUS.md), the [connective index](02_CONTEXT_AND_TRACEABILITY/REQUIREMENT_TRACEABILITY.md), and the top-level [README.md](README.md); rehash `MANIFEST.json`; commit. |
