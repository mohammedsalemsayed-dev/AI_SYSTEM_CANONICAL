...
> **Cross-reference**
> - Role: Build plan for the security/authority milestone — replace the slice's `AllowAllPolicy` and temp-dir execution with a real policy engine, scoped capabilities, structural taint, an egress broker, and a Tier-A sandbox.
> - Authority: Implementation plan; subordinate to the Complete Claude-Code Spec and [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md).
> - Upstream (consumes): [IMPLEMENTATION_PLAN.md](03_IMPLEMENTATION_FOUNDATION/prior_foundation/docs/implementation/IMPLEMENTATION_PLAN.md) milestone C; [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §4 (control loops), §12 (injection model), §14.3 (taint + policy static check + egress broker), §14.6 (tiered sandbox); [ACCEPTANCE.md](03_IMPLEMENTATION_FOUNDATION/prior_foundation/docs/testing/ACCEPTANCE.md) Security gate.
> - Downstream (depended on by): safe `code_edit_local` / `code_edit_broad` / `ops` on real repos; Milestone E (research introduces real untrusted content); the `CD-*` capability domains.
> - Predecessor: [MILESTONE_B_PLAN.md](MILESTONE_B_PLAN.md) — this milestone hardens the seams B left as stubs.
...
C fills the seams named in [milestone_b/README.md](milestone_b/README.md): the policy +
...
Guiding rule ([DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §4, §14.3): **the LLM proposes;
...
The [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §1 flow, with the `preflight` and `execute`
stages made real:
...
Tier-A ([DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §14.6) is "rootless Linux container".
...
| 15 | Full regression pass; wire everything into the orchestrator; update [IMPLEMENTATION_STATUS.md](02_CONTEXT_AND_TRACEABILITY/IMPLEMENTATION_STATUS.md) + the connective index; write `milestone_b/MILESTONE_C_NOTES.md` (what is real, seams remaining). |

## 7. Acceptance criteria

Gate order ([ACCEPTANCE.md](03_IMPLEMENTATION_FOUNDATION/prior_foundation/docs/testing/ACCEPTANCE.md)):
...
- `milestone_b/MILESTONE_C_NOTES.md` — what is real after C, what remains stubbed.
