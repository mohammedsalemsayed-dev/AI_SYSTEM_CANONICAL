...
| Matrix runner | `tests/fault/run_fault_suite.py` — runs every `(kind, injection_point)` pair over a canonical task, tallies pass/fail on the 3 invariants, writes `../nexus/FAULT_FINDINGS.md`. Offline (scripted providers) — runs in CI. |
...
../nexus/FAULT_FINDINGS.md   (generated)
```
...
| 10 | Regression; `../nexus/MILESTONE_Q_NOTES.md` (incl. every hardening fix); update [STATUS.md](../STATUS.md), the [connective index](../requirements.md), and the top-level [README.md](../../README.md); commit. |
