...
> **Cross-reference**
> - Role: Build plan for hierarchical memory (working / project / experience / system), scoped trust-filtered retrieval, a context builder, and the controlled experience lifecycle.
> - Authority: Implementation plan; subordinate to the Complete Claude-Code Spec and [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md).
> - Upstream (consumes): [IMPLEMENTATION_PLAN.md](03_IMPLEMENTATION_FOUNDATION/prior_foundation/docs/implementation/IMPLEMENTATION_PLAN.md) milestone F; [CANONICAL_CONTEXT.md](02_CONTEXT_AND_TRACEABILITY/CANONICAL_CONTEXT.md) (memory + learning); [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §7.4 (memory retention), §8 (experience promotion gates), §14.7 (retrieve-don't-classify).
> - Downstream (depended on by): Milestone G (benchmark DB / routing stats live in system memory), Milestone I (offline eval reads the guardrail set + experience history).
> - Predecessors: B, C, D, E. Continues the `milestone_b/` tree.
...
| 14 | Regression; wire-up; `milestone_b/MILESTONE_F_NOTES.md`; update [IMPLEMENTATION_STATUS.md](02_CONTEXT_AND_TRACEABILITY/IMPLEMENTATION_STATUS.md) + the connective index. |
...
- Test suite: the current 219 green, plus unit (store / retrieval / context / signature /
  ...
  decision).
- `milestone_b/MILESTONE_F_NOTES.md`.
