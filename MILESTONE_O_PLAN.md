...
> **Cross-reference**
> - Role: Build plan for the sixth and final §10.2 capability domain — a fitted routing-weight model and a per-`task_class` controller that flips routing from the static table to data-driven once the evidence supports it.
> - Authority: Implementation plan; subordinate to the Complete Claude-Code Spec and [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md).
> - Upstream (consumes): [DESIGN_TIGHTENING.md](DESIGN_TIGHTENING.md) §7.2 (score weights are **fit** — logistic regression, features → verified success, quarterly — not hand-tuned; nothing ships with magic-number weights unless tagged `provisional`), §10.2 (capability domain 6: *Automated model selection → data-driven routing replaces the static table*, "prereq: G with ≥ 20 verified runs / class"), §8 (the regression / guardrail gate a promotion passes through).
> - Downstream: none — this closes the routing loop.
> - Predecessors: G (`RouteStatsStore`, eligibility, the data-driven `Router` blend, `PROVISIONAL_WEIGHTS`), I (the guardrail regression gate the switchover is checked against). Continues the `milestone_b/` tree.
...
| 10 | Regression; `milestone_b/MILESTONE_O_NOTES.md`; update [IMPLEMENTATION_STATUS.md](02_CONTEXT_AND_TRACEABILITY/IMPLEMENTATION_STATUS.md), the [connective index](02_CONTEXT_AND_TRACEABILITY/REQUIREMENT_TRACEABILITY.md), and the top-level [README.md](README.md); rehash `MANIFEST.json`; commit. |
...
- `milestone_b/MILESTONE_O_NOTES.md`.
